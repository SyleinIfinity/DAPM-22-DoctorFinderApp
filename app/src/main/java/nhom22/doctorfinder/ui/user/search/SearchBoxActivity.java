...
public class SearchBoxActivity extends AppCompatActivity {
    ...
    private TextView chipTypeHospital;
    private TextView chipTypeClinic;
    private TextView chipTypeOnline;
    private TextView chipRatingAll;
    private TextView chipRating45;
    private TextView chipRating40;
    private TextView chipRating35;
    private TextView tvImageCount;
    private ImageView ivImage1;
    private ImageView ivImage2;
    private View btnRemoveImage1;
    private View btnRemoveImage2;
    ...
    private void initViews() {
        ...
        if (btnSearch != null) {
            btnSearch.setOnClickListener(v -> openSearchResult());
        }
    }
    ...
    private void bindEvents() {
        ...
        if (ivImage1 != null) {
            ivImage1.setOnClickListener(v -> openImagePicker());
        }
        if (ivImage2 != null) {
            ivImage2.setOnClickListener(v -> openImagePicker());
        }
        if (btnRemoveImage1 != null) {
            btnRemoveImage1.setOnClickListener(v -> removeImageAt(0));
        }
        if (btnRemoveImage2 != null) {
            btnRemoveImage2.setOnClickListener(v -> removeImageAt(1));
        }
    }
    ...
    private void refreshTypeChips() {
        applyTypeChipState(chipTypeAll, "Tất cả".equals(selectedDoctorType));
        applyTypeChipState(chipTypeHospital, "Bệnh viện".equals(selectedDoctorType));
        applyTypeChipState(chipTypeClinic, "Phòng khám".equals(selectedDoctorType));
        applyTypeChipState(chipTypeOnline, "Online".equals(selectedDoctorType));
    }

    private void refreshRatingChips() {
        applyRatingChipState(chipRatingAll, selectedMinRating == 0f);
        applyRatingChipState(chipRating45, selectedMinRating == 4.5f);
        applyRatingChipState(chipRating40, selectedMinRating == 4.0f);
        applyRatingChipState(chipRating35, selectedMinRating == 3.5f);
    }

    private void applyTypeChipState(TextView chip, boolean selected) {
        if (chip == null) {
            return;
        }
        chip.setBackgroundResource(selected ? R.drawable.bg_chip_active : R.drawable.bg_chip_inactive);
        chip.setTextColor(ContextCompat.getColor(this, selected ? R.color.white : R.color.text_muted));
    }

    private void applyRatingChipState(TextView chip, boolean selected) {
        if (chip == null) {
            return;
        }
        chip.setBackgroundResource(selected ? R.drawable.bg_chip_active : R.drawable.bg_chip_inactive);
    ...
    private void openSearchResult() {
        ...
        if (keyword.isEmpty() && symptom.isEmpty() && selectedImages.isEmpty()) {
            Toast.makeText(this, "Vui lòng nhập từ khóa, triệu chứng hoặc tải ảnh", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        intent.putStringArrayListExtra(EXTRA_IMAGE_URIS, imageUris);
